...
PSE_three=function(GT,x0,x1,confounders,V.matrix){
  ...
  return(list(omega=omega_values,
              RD=data.table::data.table(rbind(RD1,RD2,RD3,RDT),PM=c(RD1[1]/RDT[1],RD2[1]/RDT[1],RD3[1]/RDT[1],RDT[1]/RDT[1])),
              RR=data.table::data.table(rbind(RR1,RR2,RR3,RRT),PM=c(log(RR1[1])/log(RRT[1]),log(RR2[1])/log(RRT[1]),
                                                                    log(RR3[1])/log(RRT[1]),log(RRT[1])/log(RRT[1]))),
              OR=data.table::data.table(rbind(OR1,OR2,OR3,ORT),PM=c(log(OR1[1])/log(ORT[1]),log(OR2[1])/log(ORT[1]),
                                                                    log(OR3[1])/log(ORT[1]),log(ORT[1])/log(ORT[1])))))
}
