...
PSE_three=function(GT,x0,x1,confounders,V.matrix){
  ...
  return(list(omega=omega_values,RD=data.table::data.table(rbind(RD1,RD2,RD3)),
              RR=data.table::data.table(rbind(RR1,RR2,RR3)),OR=data.table::data.table(rbind(OR1,OR2,OR3))))
}
