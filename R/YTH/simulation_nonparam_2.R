...
#setwd("C:/Users/ythuang/Desktop/SI_mediation/simulation/")
...
for (k in 1:n.sim){
	...
	z <- runif(n, -0.5, 1.5)
	m1 <- a10+a11*z+rnorm(n)
	m2 <- a20+a21*z+rnorm(n)
	m3 <- a30+a31*z+rnorm(n)
	
	if (type==1) y <- 1*z + b1*m1 + b2*m2 + b3*m3 + rnorm(n)
	if (type==2) y <- exp(1*z + b1*m1 + b2*m2 + b3*m3) + rnorm(n)
	if (type==3) y <- (1*z + b1*m1 + b2*m2 + b3*m3)^2 + rnorm(n)

	bwy <- npindexbw(formula=y~z+m1+m2+m3)
	
	npmed<-function(za=0, zb=0){
	
		dat1 <- data.frame(z=za, m1=m1, m2=m2, m3=m3)
		ghat<-predict(npindex(bwy, bw=bwy), newdata=dat1)
		bwm <- npregbw(formula=ghat~z)
		dat2 <- data.frame(z=rep(zb, bwm$nobs))
		EY.zazb<-predict(npreg(bwm, bw=bwm), newdata=dat2)[1]
...
save(list = ls(all.names = TRUE), file = paste("sim_type", type, "_n", n, "_m0.RData", sep=""), envir = .GlobalEnv)
